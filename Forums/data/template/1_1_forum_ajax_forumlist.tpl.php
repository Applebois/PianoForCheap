...
<div class="mbm">Search Forum: <input type="text" class="px vm" onkeyup="forumlistsearch(this.value)" /></div>
<ul class="jump_bdl cl">
<li>
<p class="bbda xg1">All Forums</p><?php if(is_array($forumlist)) foreach($forumlist as $upfid => $gdata) { if($gdata['sub']) { ?>
...
<li>
<p class="bbda xg1">Recently Viewed</p><?php if(is_array($visitedforums)) foreach($visitedforums as $fid => $forumname) { ?><p<?php if($_GET['jfid'] == $fid) { ?> class="a"<?php } ?>><a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $fid;?>"><?php echo $forumname;?></a></p>
<?php } ?>
</li>

<li>
<p class="bbda xg1">My Favorites</p><?php if(is_array($favforums)) foreach($favforums as $forum) { ?><p<?php if($_GET['jfid'] == $forum['id']) { ?> class="a"<?php } ?>><a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $forum['id'];?>"><?php echo $forum['title'];?></a></p>
<?php } ?>
</li>
