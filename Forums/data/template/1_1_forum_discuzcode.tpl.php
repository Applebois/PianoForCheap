...
$return .= <<<EOF
, This post hidden content requires integral higher than {$creditsrequire} May browse your current integral {$_G['member']['credits']}</div>
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
<div class="locked">The following need Points higher than {$creditsrequire} May browse</div>
{$message}<br /><br />
...
?><?php
$return = <<<EOF
<div class="blockcode"><div id="{$randomid}"><ol><li>{$code}</ol></div><em onclick="copycode($('{$randomid}'));">Copy the code</em></div>
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
<div class="showhide"><h4>This post hidden content</h4>\\1</div>
...
$return .= <<<EOF
, If you want to see this please <a href="forum.php?mod=post&amp;action=reply&amp;fid={$_G['fid']}&amp;tid={$_G['tid']}" onclick="showWindow('reply', this.href)">Reply</a></div>
EOF;
?><?php 
return $return;
}
...
<div class="tip_c">
<p class="y">{$attach['dateline']} Uplaod</p>
...
EOF;
 if($attach['readperm']) { 
$return .= <<<EOF
, Read Perm: <strong>{$attach['readperm']}</strong>
EOF;
 } 
$return .= <<<EOF
, Down times: {$attach['downloads']}
EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
...
<em class="xg1">({$attach['attachsize']}, Down times: {$attach['downloads']})</em>
</p>
<div class="tip tip_4" id="aid{$attach['aid']}_menu" style="display: none" disautofocus="true">
<div>
<p>
<a href="forum.php?mod=attachment{$is_archive}&amp;aid={$aidencode}&amp;nothumb=yes" target="_blank">Download attach</a>

EOF;
 if(helper_access::check_module('album')) { 
$return .= <<<EOF

&nbsp;<a href="javascript:;" onclick="showWindow(this.id, this.getAttribute('url'), 'get', 0);" id="savephoto_{$attach['aid']}" url="home.php?mod=spacecp&amp;ac=album&amp;op=saveforumphoto&amp;aid={$attach['aid']}&amp;handlekey=savephoto_{$attach['aid']}">Save to album</a>

EOF;
 } if($firstpost && $_G['fid'] && $_G['forum']['picstyle'] && ($_G['forum']['ismoderator'] || $_G['uid'] == $attach['uid'])) { 
$return .= <<<EOF

&nbsp;<a href="forum.php?mod=ajax&amp;action=setthreadcover&amp;aid={$attach['aid']}&amp;fid={$_G['fid']}" onclick="showWindow('setcover{$attach['aid']}', this.href)">Set the cover</a>
...
<p>
<span class="y">{$attach['dateline']} Uplaod</span>
...
<br>
<a href="member.php?mod=logging&amp;action=login" onclick="showWindow('login', this.href+'&referer='+encodeURIComponent(location));">Login / Register can view big image</a>
</div>
...
<div class="xs0">
<p><strong>{$attach['filename']}</strong> <em class="xg1">({$attach['attachsize']}, Down times: {$attach['downloads']})</em></p>
<p>
<a href="forum.php?mod=attachment{$is_archive}&amp;aid={$aidencode}&amp;nothumb=yes" target="_blank">Download attach</a>
...
<p class="xg1 y">{$attach['dateline']} Uplaod</p>
...
<em class="xg1">({$attach['attachsize']}, Down times: {$attach['downloads']}
EOF;
 if($attach['price']) { 
$return .= <<<EOF
...
<div class="tip_c xs0">
<div class="y">{$attach['dateline']} Uplaod</div>
