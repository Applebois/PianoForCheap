...
0
|| checktplrefresh('./template/default/forum/post_infloat.htm', './template/default/common/seditor.htm', 1511325697, '1', './data/template/1_1_forum_post_infloat.tpl.php', './template/default', 'forum/post_infloat')
|| checktplrefresh('./template/default/forum/post_infloat.htm', './template/default/common/seccheck.htm', 1511325697, '1', './data/template/1_1_forum_post_infloat.tpl.php', './template/default', 'forum/post_infloat')
;?><?php include template('common/header'); ?><h3 class="flb">
<em id="return_<?php echo $_GET['handlekey'];?>">
<?php if($_GET['action'] == 'newthread') { ?>Post a Topic<?php } elseif($_GET['action'] == 'reply') { ?>Participation / Reply Topic<?php } ?>
</em>
<?php if($_GET['action'] == 'newthread' && $modnewthreads) { ?><span class="needverify">Required audit</span><?php } if($_GET['action'] == 'reply' && $modnewreplies) { ?><span class="needverify">Required audit</span><?php } ?>
<span>
<a href="javascript:;" class="flbc" onclick="hideWindow('<?php echo $_GET['handlekey'];?>')" title="Close">Close</a>
</span>
</h3>
...
<?php if(!$sortid) { ?><option value="0">Classified info</option><?php } if(is_array($threadsorts['types'])) foreach($threadsorts['types'] as $tsortid => $name) { if(!empty($modelid) && $threadsorts['modelid'][$tsortid] == $modelid || empty($modelid)) { ?>
...
<?php } else { ?>
<span id="subjecthide" class="z">RE: <?php echo $thread['subject'];?> [<a href="javascript:;" onclick="display('subjecthide');display('subjectbox');$('subject').value='RE: <?php echo dhtmlspecialchars(str_replace('\'', '\\\'', $thread['subject'])); ?>'">Modify</a>]</span>
...
<option value="1"<?php if($stand == 1) { ?> selected<?php } ?>>Square</option>
<option value="2"<?php if($stand == 2) { ?> selected<?php } ?>>Anti-party</option>
</select>
</div>
...
<a href="javascript:;" id="custominfo" class="y" style="margin-left:5px"><img src="<?php echo IMGDIR;?>/info_small.gif" alt="Faq" /></a>
<?php } ?>
<a href="home.php?mod=spacecp&amp;ac=credit&amp;op=rule&amp;fid=<?php echo $_G['fid'];?>" class="y" target="_blank">This forum Credits Rules</a>
<button type="submit" id="postsubmit" class="pn pnc z" value="true" name="<?php if($_GET['action'] == 'newthread') { ?>topicsubmit<?php } elseif($_GET['action'] == 'reply') { ?>replysubmit<?php } ?>" tabindex="23"><span><?php if($_GET['action'] == 'newthread') { ?>Post a Topic<?php } elseif($_GET['action'] == 'reply') { ?>Participation / Reply Topic<?php } ?></span></button>
