...
<h3 class="flb">
<em id="return_mods">The selected <?php echo $modpostsnum;?> posts</em>
<span>
...
<td class="hasd">
<label onclick="switchitemcp('itemcp_stick')" class="labeltxt">Top</label>
<div class="dopt">
...
<td>
<p class="hasd">
<label for="expirationstick" class="labeltxt">Valid</label>
...
<option value="1" <?php echo $digestcheck['1'];?>>Digest 1</option>
<?php if($_G['group']['allowdigestthread'] >= 2) { ?>
<option value="2" <?php echo $digestcheck['2'];?>>Digest 2</option>
<?php if($_G['group']['allowdigestthread'] == 3) { ?>
<option value="3" <?php echo $digestcheck['3'];?>>Digest 3</option>
<?php } } ?>
</select>
</div>
</td>
...
<td>
<p class="hasd">
<label for="expirationdigest" class="labeltxt">Valid</label>
...
<td class="hasd">
<label onclick="switchitemcp('itemcp_bump')" class="labeltxt">Up</label>
</td>
</tr>
<tr class="dopt">
<td>&nbsp;</td>
<td>
<p class="hasd">
<label for="expirationbump" class="labeltxt">Valid</label>
...
<td class="hasd"><?php $_G['forum_colorarray'] = array(1=>'#EE1B2E', 2=>'#EE5023', 3=>'#996600', 4=>'#3C9D40', 5=>'#2897C5', 6=>'#2B65B7', 7=>'#8F2A90', 8=>'#EC1282');?><label onclick="switchitemcp('itemcp_highlight')" class="labeltxt">highlight</label>
<div class="dopt">
<span class="hasd">
...
<a href="javascript:;" id="highlight_op_1" onclick="switchhl(this, 1)" class="dopt_b<?php if($stylecheck['1']) { ?> cnt<?php } ?>" style="text-indent:0;text-decoration:none;font-weight:700;" title="Text bold">B</a>
<a href="javascript:;" id="highlight_op_2" onclick="switchhl(this, 2)" class="dopt_i<?php if($stylecheck['2']) { ?> cnt<?php } ?>" style="text-indent:0;text-decoration:none;font-style:italic;" title="Italic">I</a>
<a href="javascript:;" id="highlight_op_3" onclick="switchhl(this, 3)" class="dopt_l<?php if($stylecheck['3']) { ?> cnt<?php } ?>" style="text-indent:0;text-decoration:underline;" title="Underlined">U</a>
</div>
</td>
</tr>
<tr class="dopt">
...
<td>
<p class="hasd">
<label for="expirationhighlight" class="labeltxt">Valid</label>
...
<td>
<p class="hasd">
<label class="labeltxt">Background color:</label>
...
<label><input type="radio" name="isrecommend" class="pr" value="0" />lifted</label>
</div>
</td>
</tr>
<tr class="dopt">
...
<td>
<p class="hasd">
<label for="expirationrecommend" class="labeltxt">Valid</label>
...
<td>&nbsp;</td>
<td>
<label for="reducetitle" class="labeltxt">title</label>
...
<td>&nbsp;</td>
<td>
<label class="labeltxt">Picture</label>
<select name="selectattach" onchange="updateimginfo(this.value)" class="ps" style="width: 132px">
<option value="">Not display</option><?php if(is_array($imgattach)) foreach($imgattach as $imginfo) { ?><option value="<?php echo $imginfo['aid'];?>"<?php if($selectattach == $imginfo['aid']) { ?> selected="selected"<?php } ?>><?php echo $imginfo['filename'];?></option>
<?php } ?>
</select>
</td>
</tr>
...
<p class="mbn tahfx">
Target forum: <select name="moveto" id="moveto" class="ps vm" onchange="ajaxget('forum.php?mod=ajax&action=getthreadtypes&fid=' + this.value, 'threadtypes');if(this.value) {$('moveext').style.display='';} else {$('moveext').style.display='none';}">
...
<li class="wide"><label><input type="radio" name="type" class="pr" value="normal" checked="checked" />Move Topic</label></li>
<li class="wide"><label><input type="radio" name="type" class="pr" value="redirect" />Retain steering</label></li>
</ul>
<?php } else { if($typeselect) { ?>
<input type="hidden" name="operations[]" value="type" />
<p>Category: <?php echo $typeselect;?></p>
<?php } else { ?>
Category no type settings, contact the administrator to set the theme type background<?php $hiddensubmit = true;?><?php } } ?>
</div>
...
<p>Are you sure you want to <strong> del </strong> Select theme it?</p>
<?php } else { ?>
<p>You do not have to del this topic permissions</p>
<?php } ?>
</li>
...
<td class="hasd"><label onclick="switchitemcp('itemcp_bump');" class="labeltxt" style="width:50px;margin:5px 0 10px 0px;">Up the theme</label></td>
</tr>
<tr class="dopt">
<td>&nbsp;</td>
<td>
...
<td class="hasd"><label onclick="switchitemcp('itemcp_down');" class="labeltxt" style="width:50px;margin:5px 0 10px 0px;">Sinking theme</label></td>
</tr>
<!--
<tr class="dopt">
<td>&nbsp;</td>
<td>
<p class="hasd">
<label for="expirationdown" class="labeltxt">Valid</label>
...
<ul class="llst">
<li class="wide"><label><input type="radio" name="operations[]" class="pr" value="open" <?php echo $closecheck['0'];?>  onclick="$('expirationclose').value='';" />Open Topic</label></li>
<li class="wide"><label><input type="radio" name="operations[]" class="pr" value="close" <?php echo $closecheck['1'];?> />Closed Thread</label></li>
</ul>
</td>
</tr>
</table>
...
<p class="mbn tahfx">
Target forum: <select id="moveto" name="moveto" class="ps vm">
<?php echo $forumselect;?>
</select>
</p>
<?php } ?>
...
<h4 class="cl"><a onclick="showselect(this, 'reason', 'reasonselect')" class="dpbtn" href="javascript:;">^</a><span>The reason for the action:</span></h4>
<p>
...
<button type="submit" name="modsubmit" id="modsubmit" class="pn pnc" value="Confirm"><span>Confirm</span></button>
</p>
<?php } ?>
</form>
</div>
